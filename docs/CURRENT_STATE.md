...
3. **Phase 9 in progress.** Idempotence lesson done (EV-P9-IDEMPOTENCE-470 .. EV-P9-DEDUP-TRANSFER-472): an ID names a thing, not an event; ID-only dedup is safe only when the id is unique per real event and reused by its retries. **Learner-led design, under review, not approved** (EV-P9-DESIGN-473):
   ...
   - **Resume at (patch 1 design, posted, unanswered):** blank lines in the read range: (a) name them as invalid JSON, (b) skip silently, (c) change the hook to stop writing them; choice / why / downside. Next after that: valid JSON that `parse_post_tool_use` rejects with ValueError (unsupported tool, missing field). Then write patch 1.
