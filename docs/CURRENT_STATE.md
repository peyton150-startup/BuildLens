...
3. **Phase 9 in progress.** Idempotence lesson done (EV-P9-IDEMPOTENCE-470 .. EV-P9-DEDUP-TRANSFER-472): an ID names a thing, not an event; ID-only dedup is safe only when the id is unique per real event and reused by its retries. **Learner-led design, under review, not approved** (EV-P9-DESIGN-473):
   ...
   - **Resume at:** patch 3, the D6 coverage policy. Contract posted. Design question posted and answered "no idea" (2026-09-16): where D6 lives, (a) inside `reconcile` (pass claims + verdicts) or (b) a step before `reconcile` that builds the skip set from holding Write claims, leaving `reconcile` unchanged. Next: R0 on what `reconcile` currently needs and imports (two pictures + a set of path strings; no claim or compare imports), then re-ask.
